    ...
    <div class="container mx-auto p-8 bg-gray-100 mt-4 rounded-lg">
        ...
        <div class="bg-white p-8 rounded-xl shadow-lg mb-10">
            ...
            <div class="space-y-6">
                @foreach ($addresses as $address)
                    <div
                        class="border p-6 rounded-xl flex items-center hover:bg-gray-50 transition-shadow duration-300 ease-in-out shadow-sm hover:shadow-md">
                        <input type="radio" name="address" id="address{{ $loop->index }}" value="{{ $address->id }}"
                            class="mr-4 text-primary focus:ring-primary">
        ...
        <div class="bg-white p-8 rounded-xl shadow-lg">
            ...
            <form method="POST" action="{{ route('order.buy') }}" id="checkoutForm">
                ...
                <button type="submit"
                    class="w-full bg-primary text-white py-3 px-6 rounded-lg hover:bg-tertiary transition-transform transform hover:scale-105 duration-300 shadow-lg hover:shadow-xl flex items-center justify-center">
                    @lang('messages.pay')
                </button>
            </form>
        </div>
