        ...
        <div class="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
            <div class="rounded-lg md:w-2/3">
                ...
                @foreach ($productos as $product)
                    <div class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start">
                        <img src="{{ asset('images/products/product1.jpg') }}" alt="product-image"
                        ...
                        <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                            ...
                            <div class="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                <div class="flex items-center border-gray-100 product-container"
                                    data-product-id="{{ $product->id }}">
                                    <button
                                        class="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-blue-500 hover:text-blue-50 decrement-btn">-</button>
                                    <input class="h-8 w-8 border bg-white text-center text-xs outline-none quantity-input"
                                        type="number" value="{{ $product->pivot->quantity }}" min="1" />
                                    <button
                                        class="cursor-pointer rounded-r bg-gray-100 py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50 increment-btn">+</button>
                                </div>
            ...
            <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                ...
                <form method="GET" action="{{ route('cart.checkout') }}">
                    <button type="submit"
                        class="mt-6 w-full rounded-md  py-1.5 font-medium text-blue-50 hover:bg-tertiary bg-primary">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function displayMessage(message, type) {
            ...
            messageContainer.classList.add('bg-' + (messageType === 'success' ? 'green' : 'red') + '-100',
                'border', 'border-' + (messageType === 'success' ? 'green' : 'red') + '-400', 'text-' + (
                    messageType === 'success' ? 'green' : 'red') + '-700', 'px-4', 'py-3', 'rounded',
                'relative', 'mb-4');
